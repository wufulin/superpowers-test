...
import { setView, ViewState } from '../store/slices/viewSlice'
...
export default function Header({ backView, showBack = true }: HeaderProps) {
  ...
  return (
    <header className="flex justify-between items-center px-8 py-6">
      <div className="flex items-center gap-3">
        {showBack && backView ? (
          <button
            onClick={() => dispatch(setView(backView))}
            className="w-10 h-10 flex items-center justify-center rounded-full border border-gray-200 text-gray-600 hover:bg-gray-50 transition-colors mr-2"
          >
        ...
        <span className="text-sm text-gray-500 ml-2 hidden sm:inline">时光拍立得</span>
      </div>
